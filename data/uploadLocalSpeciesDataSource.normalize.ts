...
const getCompositionGroup = (row: {
  composition_group?: unknown;
  compositionGroup?: unknown;
}) =>
  toStringValue(row.composition_group) ?? toStringValue(row.compositionGroup);

const getCompositionAxis = (row: {
  composition_axis?: unknown;
  compositionAxis?: unknown;
}): 'top' | 'bottom_left' | 'bottom_right' | null => {
  const raw =
    toStringValue(row.composition_axis) ?? toStringValue(row.compositionAxis);
  return raw && COMPOSITION_AXES.has(raw)
    ? (raw as 'top' | 'bottom_left' | 'bottom_right')
    : null;
};

const getCompositionLabel = (row: {
...
export const normalizeRawUploadedParquetBundle = (
  ...
  const densityGrid: UploadedDensityGridRow[] = (
    rawBundle.densityGrid ?? []
  ).flatMap((row): UploadedDensityGridRow[] => {
    const variable = toStringValue(row.variable);
  ...
  const occurrences = rawBundle.occurrences
    .map((row): UploadedOccurrenceRow | null => {
      ...
      return {
        ...
        // imageUrl: the canonical name util/upload.py's normalize_image_column
        // rewrites a CSV upload's own image column to. mediaUrl: the real
        // database's own column name for this same data (see
        // util/download.py -- a species-page download's occurrence.parquet
        // reuses the exact same _package_archive as uploads, but its data
        // comes from the real occurrences table, which has always called
  ...
  const categoryMetricsByVariable = [
    ...categoricalStats,
    ...ordinalStats,
  ...
  rawBundle.variableMetadata?.forEach((row) => {
    ...
    variableDefinitionsById.set(variableId, {
      ...
      legendClasses,
      compositionGroup:
        getCompositionGroup(row) ?? existing?.compositionGroup ?? null,
      compositionAxis:
        getCompositionAxis(row) ?? existing?.compositionAxis ?? null,
      compositionLabel:
        getCompositionLabel(row) ?? existing?.compositionLabel ?? null,
    });
  });
  ...
  const locations: LocationSearchResult[] = (rawBundle.locations ?? []).flatMap(
    (row) => {
