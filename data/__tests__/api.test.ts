...
describe('data/api common name normalization', () => {
  ...
  it('includes location when fetching species detail with location option', async () => {
    (global.fetch as jest.Mock).mockResolvedValue({
      ok: true,
      json: async () => ({
        taxon_id: 2,
    ...
    await fetchSpeciesByTaxonId(2, { location: 'country-us' });

    expect(global.fetch).toHaveBeenCalledWith(`${BACKEND_BASE}/api/species/2?location=country-us`);
  });
