...
// Splits a shadow string into individual layer strings while respecting parentheses in color functions.
...
// Extracts a usable color plus opacity from hex-with-alpha or rgba tokens; defaults to opaque.
...
// Converts a single shadow layer string into numeric components; returns null when color is missing.
...
// Clamps opacity values to the valid range [0, 1].
const clamp01 = (value: number) => Math.max(0, Math.min(1, value));

// Normalizes colors to rgba strings and applies per-layer opacity.
const rgbaToString = (color: string, opacity: number) => {
  ...
  // Fallback to the raw color; better to show something than nothing. Note: this does not
  // merge opacity for rgba/hsla strings (currently unnecessary because tokens are hex).
  return color;
};

/**
 * Converts a single ShadowLayer object into a CSS box-shadow layer string.
 */
const toBoxShadowLayer = (layer: ShadowLayer) =>
  `${layer.offsetX}px ${layer.offsetY}px ${layer.blurRadius}px ${layer.spreadRadius}px ${rgbaToString(layer.color, layer.opacity)}`;

// Builds React Native friendly shadow metadata (boxShadow for web/compat and elevation for Android).
export const toReactNativeShadow = (layers: ShadowLayer[]): ShadowToken => {
  ...
  const boxShadow = layers.map(toBoxShadowLayer).join(', ');
