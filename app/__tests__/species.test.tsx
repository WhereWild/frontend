  ...
  return {
    SpeciesOccurrenceMap: ({
      ...
      <View>
        ...
        <Text>{`Map heatmap tiles: ${showHeatmapTileOverlay ? 'on' : 'off'}`}</Text>
      </View>
    ),
  };
});
...
describe('Species screen', () => {
  ...
  it('forwards both heatmap toggles to the map component', async () => {
    render(<SpeciesScreen data={createData()} />);

    await waitForSpeciesEffectsToSettle();

    fireEvent.press(screen.getByLabelText('Prediction Overlay'));
    fireEvent.press(screen.getByLabelText('Prediction Tiles'));
