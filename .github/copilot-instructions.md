...
Always import design tokens from `constants/theme.ts` and render copy with `ThemedText`:
...
> ℹ️ **Index `Size` tokens with strings.** Always use bracket notation with string keys (e.g., `Size.space['600']`, `Size.radius['200']`) instead of numeric access. This keeps us aligned with how the tokens are defined and avoids TypeScript inference issues.

> ℹ️ **Always render text with `ThemedText`.** Direct `<Text>` usage should only happen inside `ThemedText` itself.
...
1. ✅ **Use React Native components + `ThemedText`** - `View`, `Pressable` for layout/interaction, and `ThemedText` for all copy
...
6. ✅ **Respect component defaults** - Only pass props when the design explicitly deviates from the shared component’s default behavior or copy
7. ✅ **Keep implementations KISS** - Favor the simplest approach that meets requirements before adding abstractions
