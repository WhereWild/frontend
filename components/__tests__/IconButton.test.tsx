...
const createIconProbe = () => {
  ...
  const IconProbe = (props: { color?: string; size?: string }) => {
    calls.push(props);
    return <Text testID="icon-button-probe" />;
  };
...
describe('IconButton Component', () => {
  ...
  describe('Icons', () => {
    ...
    it('applies small icon size token when size prop is small', () => {
      const { IconProbe, calls } = createIconProbe();
      ...
      render(
        <IconButton
          size="small"
          icon={<IconProbe />}
          accessibilityLabel="Small Icon Size"
        />
      );
    ...
    it('matches snapshot (smoke test)', () => {
      const tree = render(
        <IconButton
          ...
          size="medium"
          icon={<MockIcon />}
          accessibilityLabel="Primary Medium"
        />
      ).toJSON();
