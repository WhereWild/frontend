...
export function useNavigationBarPanResponder({
  ...
  const setPreviewIndexTracked = React.useCallback((index: number | null) => {
  ...
  const updatePreviewAtPagePoint = React.useCallback((pageX: number, pageY: number) => {
    ...
    const hoveredTabIndex = getTabIndexAtPoint(pageX - origin.x, pageY - origin.y);
  ...
  const updatePreviewAtEventPoint = React.useCallback((event: GestureResponderEvent) => {
  ...
  const panResponder = React.useMemo(() => PanResponder.create({
    ...
    onPanResponderRelease: (_event, gestureState) => {
      ...
      const releasedTabIndex = updatePreviewAtPagePoint(gestureState.moveX, gestureState.moveY)
