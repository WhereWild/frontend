...
describe('getCbColor', () => {
  it('returns fallback when cbMode is null', () => {
    expect(getCbColor('kg0', 1, null, '#aabbcc')).toBe('#aabbcc');
  });

  it('returns fallback when cbMode is undefined', () => {
    expect(getCbColor('kg0', 1, undefined, '#aabbcc')).toBe('#aabbcc');
  });

  it('returns cb color for known variable and class', () => {
    const result = getCbColor('kg0', 1, 'colorblind', '#fallback');
  ...
  it('returns fallback for unknown class id', () => {
    expect(getCbColor('kg0', 9999, 'colorblind', '#fallback')).toBe(
      '#fallback',
    );
  });

  it('strips temporal suffix before lookup', () => {
    const withSuffix = getCbColor('kg0_avg_168h', 1, 'colorblind', '#fallback');
    const withoutSuffix = getCbColor('kg0', 1, 'colorblind', '#fallback');
    expect(withSuffix).toBe(withoutSuffix);
  });

  it('returns color for achromatopsia mode', () => {
    const result = getCbColor('kg0', 1, 'achromatopsia', '#fallback');
    expect(result).not.toBe('#fallback');
  });
});
...
describe('getCbShape', () => {
  ...
  it('returns circle as fallback for unknown class id', () => {
    expect(getCbShape('kg0', 9999)).toBe('circle');
  });

  it('strips temporal suffix before lookup', () => {
    const withSuffix = getCbShape('kg0_mode_168h', 1);
    const withoutSuffix = getCbShape('kg0', 1);
    expect(withSuffix).toBe(withoutSuffix);
  });
});
