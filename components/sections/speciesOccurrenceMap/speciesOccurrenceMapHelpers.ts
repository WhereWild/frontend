...
// The one place that decides what an observation's dot looks like for a
// given variable — shared by preparePointsForMapHtml (initial page build)
// and computePointStyleUpdates (live variable-switch updates, no reload),
// so the two can never drift into disagreeing about a point's color/shape.
const computePointVarFields = (
  catalog: string,
  ...
  return (points ?? []).map((point) => {
    ...
    const catalog =
      typeof catalogNumber === 'number' || typeof catalogNumber === 'string'
        ...
        : '';
    const { varValue, varColor, varLabel, varShape } = computePointVarFields(
      catalog,
      observationValues,
      classColors,
      classLabels,
    ...
    const mediaUrl =
      typeof point.mediaUrl === 'string' && point.mediaUrl ? point.mediaUrl : null;
    ...
    return {
      ...
      popupMediaAttribution: mediaAttribution ? escapeHtml(mediaAttribution) : null,
      popupMediaLicense: mediaLicense ? escapeHtml(mediaLicense) : null,
      popupMediaLicenseUrl: mediaLicenseUrl ? escapeHtml(mediaLicenseUrl) : null,
      varValue,
      varColor,
      varLabel,
      varShape,
...
export type PointStyleUpdate = PointVarFields & { catalog: string };

// The live-update counterpart to preparePointsForMapHtml: instead of
// building full renderable point objects (baked into the initial HTML),
// this produces the minimal per-point payload needed to recolor/reshape
// already-rendered markers in place — sent via postMessage
// (POINT_STYLES_UPDATE_MESSAGE_TYPE) whenever the selected variable
// changes but the underlying occurrences (positions) don't, so switching
// variables never has to reload the whole map.
export const computePointStyleUpdates = (
  points: Record<string, unknown>[],
  observationValues?: Map<string, number> | null,
  classColors?: Map<string, string> | null,
  classLabels?: Map<string, string> | null,
  ...
  return (points ?? []).reduce<PointStyleUpdate[]>((acc, point) => {
    const catalogNumber = point.catalogNumber;
    const catalog =
      typeof catalogNumber === 'number' || typeof catalogNumber === 'string'
        ? String(catalogNumber)
