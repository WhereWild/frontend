    ...
    function evaluateOfflineFallback() {
      ...
      if (cacheToggleBtn) { cacheToggleBtn.style.display = shouldShowFallback ? '' : 'none'; }
    ...
    // https://github.com/w3c/ServiceWorker/issues/1390) — Leaflet's plain
    // <img> tile requests are already served from the browser's own native
    // HTTP cache (governed by the tile responses' Cache-Control headers),
    // which is what makes previously-browsed areas work offline here. This
    // toggle controls that directly: disabling it appends a cache-busting
    // query param to every tile URL, forcing a genuine network fetch that
    // bypasses the browser cache entirely (and therefore fails while
    // offline, even for a previously-cached tile) — the same technique
    // browsers use internally for cache-busting, just applied deliberately
    // so this is actually verifiable while testing.
    ...
    var originalGetTileUrl = tileLayer.getTileUrl.bind(tileLayer);
    tileLayer.getTileUrl = function(coords) {
      var url = originalGetTileUrl(coords);
      if (cacheFallbackDisabled) {
    ...
    function setCacheFallbackDisabled(disabled) {
      ...
      // Already-rendered tiles don't re-request themselves just because
      // getTileUrl changed — redraw() forces every visible tile to be
      // recreated (and thus re-requested) under the new setting right away,
      // instead of only affecting tiles loaded after the next pan/zoom.
      tileLayer.redraw();
    }
    // Only visible while the fallback UI itself is showing (i.e. offline) —
    // hidden the rest of the time so it doesn't clutter the controls for
    // regular online use. Display toggling happens in evaluateOfflineFallback
    // above via cacheToggleBtn.
    var cacheToggleBtn = null;
    if (!window.ReactNativeWebView) {
      var CacheToggleControl = L.Control.extend({
        options: { position: 'topright' },
        onAdd: function() {
          var btn = L.DomUtil.create('button', 'leaflet-locate-btn');
          btn.style.display = shouldShowFallback ? '' : 'none';
      ...
      // Deferred: this partial is injected right after tileLayer.addTo(map),
      // well before the main template's later controls (fullscreen, globe
      // toggle, basemap mode). Adding this control synchronously here would
      // make it the FIRST control in the topright stack — Leaflet stacks
      // same-corner controls in add order, first-added on top — putting a
      // debug-only button above real controls. Deferring to a macrotask runs
      // this after all of those have already been added, placing it last
      // (bottom of the stack) instead.
      setTimeout(function() {
        map.addControl(new CacheToggleControl());
      }, 0);
    }
