...
export type CircularColormapId =
  | 'twilight'
  | 'twilight_90'
  | 'twilight_180'
  | 'twilight_270';
...
function makeConicCss(stops: [number, number, number][]): string {
  ...
  const parts = stops.map(
    (s, i) => `rgb(${s[0]},${s[1]},${s[2]}) ${Math.round((i / n) * 360)}deg`,
  );
  const first = stops[0];
...
export const CIRCULAR_COLORMAP_ORDER: CircularColormapId[] = [
  'twilight',
  'twilight_90',
  'twilight_180',
  'twilight_270',
