...
function HookProbe(props: Parameters<typeof useEnvironmentVariableSelection>[0]) {
  ...
  return (
    <View>
      ...
      <Text testID="selected-category">{state.selectedVariableCategory ?? 'null'}</Text>
      <Text testID="selected-variable">{state.selectedVariable}</Text>
      <Text testID="selected-meta">{state.selectedVariableMeta?.id ?? 'null'}</Text>
...
describe('useEnvironmentVariableSelection', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockFetchEnvironmentVariables.mockResolvedValue([] as never);
  });
  ...
  it('selects the first available category and variable when the initial variable is absent', async () => {
    const { result } = renderHook(() =>
      useEnvironmentVariableSelection({
        variableId: 'unknown',
        variables: [
          { id: 'wind_speed', label: 'Wind Speed', category: 'Live Weather', valueType: 'continuous' },
          { id: 'landcover', label: 'Land Cover', category: 'Categorical', valueType: 'categorical' },
        ],
      }),
    );
    ...
    expect(result.current.filteredVariables.map((item) => item.id)).toEqual(['landcover']);
  ...
  it('filters excluded categories from the remote catalog', async () => {
    mockFetchEnvironmentVariables.mockResolvedValueOnce([
      { id: 'temporal-only', name: 'Temporal Only', category: 'Temporal', valueType: 'continuous' },
      { id: 'recent-weather', name: 'Recent Weather', category: 'Recent Weather', valueType: 'continuous' },
      { id: 'wind_speed', name: 'Wind Speed', category: 'Live Weather', valueType: 'continuous' },
    ...
    expect(screen.getByTestId('filtered-variables').props.children).toBe(JSON.stringify(['landcover']));
    expect(screen.getByTestId('selected-category').props.children).toBe('Categorical');
    expect(screen.getByTestId('selected-meta').props.children).toBe('landcover');
  });
  ...
  it('does not fall back to default variables when exclusions remove the entire remote catalog', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('filtered-variables').props.children).toBe(JSON.stringify([]));
    });
    expect(screen.getByTestId('categories').props.children).toBe(JSON.stringify([]));
