...
const DensityChart = ({
  ...
  const normalized = samples.map((sample) => ({
    ...
    y: CHART_HEIGHT - (sample.y / safeMaxY) * CHART_HEIGHT,
  }));
  ...
  return (
    ...
    >
      <View style={styles.headingRow}>
        ...
        {stats?.units || selectedVariableMeta?.units ? (
          <ThemedText variant="bodySmall">
            Units: {stats?.units ?? selectedVariableMeta?.units}
          </ThemedText>
        ) : null}
      </View>
        ...
        <>
          ...
          {showRankContext && rankContextOptions.length > 1 ? (
            <View style={styles.rankContextRow}>
              ...
              <View style={styles.rankContextOptions}>
                {rankContextOptions.map((option) => {
                  const selected = option.key === selectedRankContext;
                  return (
                    <Pressable
                      key={option.key}
                      onPress={() => setSelectedRankContext(option.key)}
                      style={[
                        styles.rankContextChip,
                        {
                          backgroundColor: selected
                            ? palette.background.brand.default
                            : palette.background.default.tertiary,
