...
import { Platform, type GestureResponderEvent, type PressableProps, type TextStyle, type ViewStyle } from 'react-native';
import { Size, Time, TimeEasingCurves } from '@/constants/theme';
import type { IconProps, IconSize } from '../../primitives/Icon';

export type ButtonIconElement = React.ReactElement<Pick<IconProps, 'color' | 'size' | 'style'>>;
export type ButtonIcon = React.ComponentType<Pick<IconProps, 'color' | 'size' | 'style'>> | ButtonIconElement;
...
export const buttonSurfaceTransitionStyle = resolveWebTransitionStyle<ViewStyle>(
  'background-color, border-color',
);

export const buttonTextTransitionStyle = resolveWebTransitionStyle<TextStyle>('color');

export const buttonIconTransitionStyle = resolveWebTransitionStyle<ViewStyle>('color, fill, stroke');
...
export const useHoverOnlyButtonTransitions = (options?: HoverOnlyTransitionOptions) => {
  const [shouldAnimateTransitions, setShouldAnimateTransitions] = React.useState(false);
...
export const renderButtonIconElement = (
  ...
  const currentProps = iconElement.props as Pick<IconProps, 'color' | 'size' | 'style'>;
  const iconTransitionStyle = getButtonIconTransitionStyle(options?.animate ?? true);
  const mergedStyle = iconTransitionStyle == null && currentProps.style == null
    ? undefined
