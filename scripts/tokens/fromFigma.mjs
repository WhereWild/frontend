...
async function fetchFigmaJson(pathname, requestDescription) {
  const fileResponse = await fetch(`${URL_BASE}${pathname}`, {
    method: "GET",
  ...
  try {
    ...
    throw new Error(
      `Figma API returned invalid JSON for ${requestDescription} ` +
    ...
    const apiMessage =
      data && typeof data === "object" && "err" in data && typeof data.err === "string"
        ...
        : data && typeof data === "object" && "message" in data && typeof data.message === "string"
          ...
          : responseText.trim();
    throw new Error(
      `Figma API request failed for ${requestDescription} ` +
      `(status ${fileResponse.status}): ${apiMessage || "Unknown error"}`,
    );
  }

  if (!data || typeof data !== "object") {
    throw new Error(`Figma API returned an empty response for ${requestDescription}.`);
  }
...
export async function getFileStyles(fileKey) {
  const data = await fetchFigmaJson(`/${fileKey}`, `file styles for ${fileKey}`);
...
export async function getFileVariables(fileKey, nameSpace) {
  const data = await fetchFigmaJson(
    `/${fileKey}/variables/local`,
    `local variables for ${fileKey}`,
  );
...
function fileRESTResponseToStylesJSON(response) {
  if (!response.styles || typeof response.styles !== "object") {
    throw new Error("Figma file response is missing styles data.");
  }
  if (!response.document || typeof response.document !== "object") {
    throw new Error("Figma file response is missing document data.");
  }
...
function variablesRESTResponseToVariablesJSON(response, nameSpace) {
  if (!response.meta || typeof response.meta !== "object") {
    throw new Error("Figma variables response is missing meta data.");
  }
  if (
    !response.meta.variableCollections ||
    typeof response.meta.variableCollections !== "object"
  ) {
    throw new Error("Figma variables response is missing variableCollections.");
  }
  if (!response.meta.variables || typeof response.meta.variables !== "object") {
    throw new Error("Figma variables response is missing variables.");
  }
